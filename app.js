(function () {
 ...
 // ====================== KONA (clean rewrite, half-pattern, 30mm top offset) ======================
function initKona() {
  const form = $("konaForm");
  if (!form) return;
  let currentRot = 0;
...
// ---- Geometry solver: slope k so top circle = Rt at 30mm above high side
function solveK_withOffset(Rt, Rb, m, H) {
  const eps = 1e-9;
  function f(k) {
    const y0 = (Rb * (1 - m * k)) / Math.max(k, eps);
    const rHigh = (k * y0) / (1 + m * k);
    const alpha = Math.atan(k);
    const sinA = Math.sin(alpha);
    return (rHigh / Math.max(sinA, eps)) + H * k - Rt;
  }
  let a = eps, b = 1.0, fa=f(a), fb=f(b), tries=0;
  while (fa*fb>0 && tries<30) { b*=1.5; fb=f(b); tries++; }
  if (fa*fb>0) return 0.5; // fallback
  for (let i=0;i<80;i++) {
...
function generateKonaPoints(topD, botD, planeDeg, rotDeg) {
  const Rt = topD/2, Rb = botD/2, H=30, N=6;
  const m = Math.tan((planeDeg*Math.PI)/180);
  const k = solveK_withOffset(Rt,Rb,m,H);
  const alpha=Math.atan(k), sinA=Math.sin(alpha), sinASafe=Math.max(sinA,1e-9);
  ...
  const ang=(rotDeg*Math.PI)/180, rot2d=([x,y])=>[x*Math.cos(ang)-y*Math.sin(ang),x*Math.sin(ang)+y*Math.cos(ang)];
  const innerR=inner.map(rot2d), outerR=outer.map(rot2d);
