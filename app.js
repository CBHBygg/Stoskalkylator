(function () {
  ...
  async function exportMultiPagePDF(previewId, filenameBase) {
    ...
    for (let r = 0; r < rows; r++) {
      for (let c = 0; c < cols; c++) {
        ...
        const body = clone.querySelector("g") || clone;
  ...
  function printSVG(previewId) {
    const svg = document.querySelector(`#${previewId} svg`);
    if (!svg) {
      alert("Ingen SVG att skriva ut.");
      return;
    }
    const win = window.open("", "_blank");
    win.document.write(`<!doctype html><title>Skriv ut</title>
      <style>html,body{margin:0;padding:0} svg{width:210mm;height:auto}</style>`);
  ...
  $("stosForm").addEventListener("submit", (e) => {
    ...
    $("stosMeta").textContent =
      `Bredd (omkrets/2) = ${L.toFixed(1)} mm · Skärningshöjd = ${Hs.toFixed(1)} mm · H(min) = ${Hmin.toFixed(1)} mm`;
  ...
  function generateKonaPoints(topD, botD, slopeDeg, rotDeg) {
    ...
    const theta_total = 2 * Math.PI * (R1 / (k * R1));
