(function () {
  ...
  // ✅ Geometry with fixed topD, botD, slopeDeg
  function generateKonaPoints(topD, botD, slopeDeg, rotDeg = 0) {
    const Rb = botD / 2;  // bottom radius (fixed, largest)
    const Rt = topD / 2;  // top radius (fixed, smaller)
    const alpha = slopeDeg * Math.PI / 180;

    // vertical height from slope
    const H = (Rb - Rt) / Math.tan(alpha);
    const s = Math.sqrt(H*H + (Rb - Rt)*(Rb - Rt));

    // development radii
    const L_out = s * (Rb / (Rb - Rt));
    const L_in  = s * (Rt / (Rb - Rt));
    const arcAngle = (2 * Math.PI * Rb) / L_out;

    const N = 100;
    const thetas = Array.from({ length: N }, (_, i) => -arcAngle/2 + i*(arcAngle/(N-1)));

    const inner = thetas.map(t => [L_in * Math.cos(t), L_in * Math.sin(t)]);
    const outer = thetas.map(t => [L_out * Math.cos(t), L_out * Math.sin(t)]);
