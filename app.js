...
(function () {
  ...
  function printSVG(previewId) {
    ...
    const html = `<!doctype html><html><head><meta charset="utf-8">
      <style>@page { size: A4; margin: 10mm; } svg{width:${w};height:${h};}</style>
  ...
  $("stosForm").addEventListener("submit", (e) => {
    ...
    $("stosMeta").textContent = `Stos: Ø=${D} mm, vinkel=${slopeDeg}°, höjd(min)=${Hmin} mm`;
  ...
  function renderKona(topD, botD, slopeDeg, rot) {
    ...
    $("konaMeta").textContent = `Kona: ToppØ=${topD} mm, BottenØ=${botD} mm, Vinkel=${slopeDeg}°, Rotation=${rot}°`;
